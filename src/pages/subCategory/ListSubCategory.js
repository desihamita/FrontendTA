...
const ListSubCategory = () => {
    ...
    const getCategories = useCallback((pageNumber = 1) => {
        setIsLoading(true);
        axios.get(`${Constants.BASE_URL}/sub-category?page=${pageNumber}&search=${input.search}&order_by=${input.order_by}&per_page=${input.per_page}&direction=${input.direction}`)
            .then(res => {
    ...
    }, [input, input.search, input.order_by, input.per_page, input.direction]);
                                    ...
                                    <tbody>
                                        {categories.length > 0 ? categories.map((category, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className='m-1'>
                                                    <button onClick={() => handleDetailsModal(category)} className='btn btn-info btn-sm my-1'>
