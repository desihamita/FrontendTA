...
const ListSubCategory = () => {
    ...
    const getCategories = (pageNumber = 1) => {
        setIsLoading(true)
        axios.get(`${Constants.BASE_URL}/sub-category?page=${pageNumber}&search=${input.search}&ordey_by=${input.order_by}&per_page=${input.per_page}&direction=${input.direction}`)
            .then(res => {
                        ...
                        <div className="card card-outline card-warning">
                            <div className="card-header">
                                <div className="d-flex justify-content-between align-items-center">
                                    <CardHeader add={'/sub-category/create'} />
                                </div>
                            </div>
