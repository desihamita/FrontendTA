...
const Profile = () => {
    ...
    const getShops = useCallback((pageNumber = 1) => {
        setIsLoading(true);
        ...
        axios.get(`${Constants.BASE_URL}/get-user`)
          .then(res => {
            if (res.data.length > 0) {
              // Assuming the user data is not used
            }
          })
          .catch(error => {
            console.log(error);
    ...
    useEffect(() => {
        getShops();
        getUser();
    }, [getShops]); // Added getShops to dependency array
                                            ...
                                            <tbody>
                                                {shops.length > 0 ? shops.map((shop, index) => (
                                                    <tr key={index}>
                                                        ...
                                                        <td className='m-1'>
                                                            <button onClick={() => handleDetailsModal(shop)} className='btn btn-info btn-sm my-1'>
